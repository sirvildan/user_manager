...
class UserManagerTest extends AnyFlatSpec with TestContainerForAll {
  ...
  "UserManager" should "work correctly" in {
    withContainers { pgContainer =>
      ...
      serviceResource
        .use { dbService =>
          ...
          dbService.createUserMeta(
            UserMeta("updatemeta@mail.ru", "old_hobby", List("OldFriend1@mail.ru", "OldFriend2@mail.ru"))
          ) *>
          dbService.updateUserMeta(
            UserMeta("updatemeta@mail.ru", "new_hobby", List("NewFriend1@mail.ru", "NewFriend2@mail.ru"))
          )
        }
        .unsafeRunSync()
      ...
      assert(
        metaupd.get.hobby == "new_hobby" && metaupd.get.friendsEmails == List(
          "NewFriend1@mail.ru",
          "NewFriend2@mail.ru"
        )
      )
