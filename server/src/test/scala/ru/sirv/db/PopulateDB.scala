...
object PopulateDB extends IOApp {
  ...
  val dbConfig: Config = Config(
    DbModule.PostgresConfig(
      DbModule.ConnectionConfig(
        "org.postgresql.Driver",
        "jdbc:postgresql://localhost:5432/sirvildan_db",
        "postgres",
        "internet"
      ),
      DbModule.PoolConfig(8, 1, 3.seconds, Some(1.minute), Some(5.minutes), 8)
    ),
